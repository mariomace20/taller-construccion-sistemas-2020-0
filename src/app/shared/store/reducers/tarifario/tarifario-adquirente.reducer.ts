...
import { TarifarioAdquirenteActions, actions } from '../../actions/tarifario/tarifario-adquirente.actions';
import { RESOURCE_ACTIONS } from '../../../utils';
import { State } from '../entity-state.model';

const INITIAL_STATE: State<TarifarioAdquirente> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function tarifarioAdquirenteReducer(state = INITIAL_STATE, action: TarifarioAdquirenteActions): State<TarifarioAdquirente> {
  switch (action.type) {
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data.findIndex(b => b.idTarifarioAdquirente === state.selected.idTarifarioAdquirente);
      if (index >= 0) {
        const data = [
    ...
    case actions.DELETE_SUCCESS: {
      const data = state.data.filter(b => b.idTarifarioAdquirente !== state.selected.idTarifarioAdquirente);
      return {
        ...state,
        data,
        selected: null,
