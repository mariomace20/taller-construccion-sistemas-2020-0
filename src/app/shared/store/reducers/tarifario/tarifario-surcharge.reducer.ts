...
import { TarifarioSurchargeActions, actions } from '../../actions/tarifario/tarifario-surcharge.actions';
import { RESOURCE_ACTIONS } from '../../../utils';
import { State } from '../entity-state.model';

const INITIAL_STATE: State<TarifarioSurcharge> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function tarifarioSurchargeReducer(state = INITIAL_STATE, action: TarifarioSurchargeActions): State<TarifarioSurcharge> {
  switch (action.type) {
    case actions.RESET:
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data.findIndex(b => b.idTarifarioSurcharge === state.selected.idTarifarioSurcharge);
      if (index >= 0) {
        const data = [
    ...
    case actions.DELETE_SUCCESS: {
      const data = state.data.filter(b => b.idTarifarioSurcharge !== state.selected.idTarifarioSurcharge);
      return {
        ...state,
        data,
        selected: null,
