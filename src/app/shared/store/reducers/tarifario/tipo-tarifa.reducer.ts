...
import { TipoTarifaActions, actions } from '../../actions/tarifario/tipo-tarifa.actions';
import { RESOURCE_ACTIONS } from '../../../utils';
import { State } from '../entity-state.model';

const INITIAL_STATE: State<TipoTarifa> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function tipoTarifaReducer(state = INITIAL_STATE, action: TipoTarifaActions): State<TipoTarifa> {
  switch (action.type) {
    case actions.RESET:
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data.findIndex(b => b.idTarifa === state.selected.idTarifa);
      if (index >= 0) {
        const data = [
    ...
    case actions.DELETE_SUCCESS: {
      const data = state.data.filter(b => b.idTarifa !== state.selected.idTarifa|| b.idEsquema !== state.selected.idEsquema);
      return {
        ...state,
        data,
        selected: null,
