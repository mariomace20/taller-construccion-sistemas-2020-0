...
import { TarifarioMiembroActions, actions } from '../../actions/tarifario/tarifario-miembro.actions';
import { RESOURCE_ACTIONS } from '../../../utils';
import { State } from '../entity-state.model';

const INITIAL_STATE: State<TarifarioMiembro> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function tarifarioMiembroReducer(state = INITIAL_STATE, action: TarifarioMiembroActions): State<TarifarioMiembro> {
  switch (action.type) {
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data.findIndex(b => b.idTarifarioMiembro === state.selected.idTarifarioMiembro);
      if (index >= 0) {
        const data = [
    ...
    case actions.DELETE_SUCCESS: {
      const data = state.data.filter(b => b.idTarifarioMiembro !== state.selected.idTarifarioMiembro);
      return {
        ...state,
        data,
        selected: null,
