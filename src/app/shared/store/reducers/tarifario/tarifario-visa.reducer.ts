...
import { TarifarioVISAActions, actions } from '../../actions/tarifario/tarifario-visa.actions';
import { RESOURCE_ACTIONS } from '../../../utils';
import { State } from '../entity-state.model';

const INITIAL_STATE: State<TarifarioVISA> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function tarifarioVISAReducer(state = INITIAL_STATE, action: TarifarioVISAActions): State<TarifarioVISA> {
  switch (action.type) {
    case actions.RESET:
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data.findIndex(b => (b.idTarifarioVisa === state.selected.idTarifarioVisa
                                                                    && b.idTipoComision === state.selected.idTipoComision) );
      if (index >= 0) {
        const data = [
    ...
    case actions.DELETE_SUCCESS: {
      const data = state.data.filter(b => (b.idTarifarioVisa !== state.selected.idTarifarioVisa
                                                                    && b.idTipoComision !== state.selected.idTipoComision));
      return {
        ...state,
        data,
