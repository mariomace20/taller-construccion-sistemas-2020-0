...
import { TarifarioEmisorActions, actions } from '../../actions/tarifario/tarifario-emisor.actions';
import { RESOURCE_ACTIONS } from '../../../utils';
import { State } from '../entity-state.model';

const INITIAL_STATE: State<TarifarioEmisor> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function tarifarioEmisorReducer(state = INITIAL_STATE, action: TarifarioEmisorActions): State<TarifarioEmisor> {
  switch (action.type) {
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data.findIndex(b => b.idTarifarioEmisor === state.selected.idTarifarioEmisor);
      if (index >= 0) {
        const data = [
    ...
    case actions.DELETE_SUCCESS: {
      const data = state.data.filter(b => b.idTarifarioEmisor !== state.selected.idTarifarioEmisor);
      return {
        ...state,
        data,
        selected: null,
