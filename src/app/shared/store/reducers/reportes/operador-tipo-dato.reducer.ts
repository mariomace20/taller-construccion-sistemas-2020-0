import { OperadorTipoDatoActions, actions } from '../../actions/reportes/operador-tipo-dato.actions';
...
const INITIAL_STATE: State<OperadorTipoDato> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function operadorTipoDatoReducer(state = INITIAL_STATE, action: OperadorTipoDatoActions): State<OperadorTipoDato> {
  switch (action.type) {
    ...
    case actions.GET_GROUP_OPERADOR_TIPO_DATO:
      return {
        ...state,
        action: RESOURCE_ACTIONS.CONSULTA,
        loading: true,
    ...
    case actions.GET_GROUP_OPERADOR_TIPO_DATO_SUCCESS:
      return {
        ...state,
        data: action.payload,
        loading: false,
    ...
    case actions.GET_GROUP_OPERADOR_TIPO_DATO_FAIL:
      return {
        ...state,
        data: [],
        loading: false,
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data.findIndex(b => b.idOperador === state.selected.idOperador);
      if (index >= 0) {
        const data = [
    ...
    case actions.DELETE_SUCCESS: {
      const data = state.data.filter(b => b.idOperador !== state.selected.idOperador);
      return {
        ...state,
        data,
        selected: null,
