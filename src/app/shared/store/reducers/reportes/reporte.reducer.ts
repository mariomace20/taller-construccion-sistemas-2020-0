...
import { ReporteActions, actions } from '../../../../shared/store/actions/reportes/reporte-estado.actions';
import { RESOURCE_ACTIONS } from '../../../utils';
import { State } from '../entity-state.model';

const INITIAL_STATE: State<ReporteAvanzadoState> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function reporteReducer(state = INITIAL_STATE, action: ReporteActions): State<ReporteAvanzadoState> {
  switch (action.type) {
    ...
    case actions.UPDATE_SUCCESS: {
      ...
      const index = state.data.findIndex(b => b.idReporte === state.selected.idReporte);
      if (index >= 0) {
        const data = [
    ...
    case actions.DELETE_SUCCESS: {
      const data = state.data.filter(b => b.idReporte !== state.selected.idReporte);
      return {
        ...state,
        data,
        selected: null,
