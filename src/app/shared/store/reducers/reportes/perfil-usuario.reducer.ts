...
import { PerfilUsuarioActions,actions } from '../../actions/reportes/perfil-usuario.actions';

const INITIAL_STATE: State<PerfilUsuario> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function perfilUsuarioReducer(state = INITIAL_STATE, action: PerfilUsuarioActions): State<PerfilUsuario> {
  switch (action.type) {
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data.findIndex(b => b.idPerfilRepUsuario === state.selected.idPerfilRepUsuario);
      if (index >= 0) {
        const data = [
    ...
    case actions.DELETE_SUCCESS: {
      const data = state.data.filter(b => b.idPerfilRepUsuario !== state.selected.idPerfilRepUsuario);
      return {
        ...state,
        data,
        selected: null,
