...
import { UsuarioActions, actions  } from '../../actions/reportes/usuario.actions';

const INITIAL_STATE: State<Usuario> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function usuarioReducer(state = INITIAL_STATE, action: UsuarioActions): State<Usuario> {
  switch (action.type) {
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data.findIndex(b => b.username === state.selected.username);
      if (index >= 0) {
        const data = [
    ...
    case actions.DELETE_SUCCESS: {
      const data = state.data.filter(b => b.username !== state.selected.username);
      return {
        ...state,
        data,
        selected: null,
