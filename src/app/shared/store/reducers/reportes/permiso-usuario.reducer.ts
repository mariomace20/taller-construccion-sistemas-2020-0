...
import { PermisoUsuarioActions, actions } from '../../actions/reportes/permiso-usuario.actions';

const INITIAL_STATE: State<PermisoUsuario> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function PermisoUsuarioReducer(state = INITIAL_STATE, action: PermisoUsuarioActions): State<PermisoUsuario> {
  switch (action.type) {
    ...
    case actions.GET_TABLAS_PERMITIDAS:
      return {
        ...state,
        action: RESOURCE_ACTIONS.CONSULTA,
        loading: true,
    ...
    case actions.GET_TABLAS_PERMITIDAS_SUCCESS:
      return {
        ...state,
        data: action.payload,
        loading: false,
    ...
    case actions.GET_TABLAS_PERMITIDAS_FAIL:
      return {
        ...state,
        data: [],
        loading: false,
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data.findIndex(b => b.idPerfil === state.selected.idPerfil);
      if (index >= 0) {
        const data = [
    ...
    case actions.DELETE_SUCCESS: {
      const data = state.data.filter(b => b.idPerfil !== state.selected.idPerfil);
      return {
        ...state,
        data,
        selected: null,
    ...
    case actions.UPDATE_ALL_SUCCESS:
      return {
        ...state,
        data: [],
        loading: false,
