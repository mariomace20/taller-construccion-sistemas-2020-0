...
import { TablaQueryActions, actions } from '../../actions/reportes/tablas-query.actions';
import { RESOURCE_ACTIONS } from '../../../utils';
import { State } from '../entity-state.model';

const INITIAL_STATE: State<TablaQuery> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function tablasQueryReducer(state = INITIAL_STATE, action: TablaQueryActions): State<TablaQuery> {
  switch (action.type) {
    ...
    case actions.GET_ALL_FAIL:
      ...
      case actions.GET_CRITERIO_FAIL:
        ...
        case actions.UPDATE_SUCCESS: {
          const index = state.data.findIndex(b => b.idTablaQuery === state.selected.idTablaQuery);
          if (index >= 0) {
            const data = [
        ...
        case actions.DELETE_SUCCESS: {
          const data = state.data.filter(b => b.idTablaQuery !== state.selected.idTablaQuery);
          return {
            ...state,
            data,
            selected: null,
