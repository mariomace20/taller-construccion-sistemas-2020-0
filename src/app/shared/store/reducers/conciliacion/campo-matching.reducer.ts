...
import { CampoMatchingActions, actions } from '../../actions/conciliacion/campo-matching.actions';
import { RESOURCE_ACTIONS } from '../../../utils';
import { State } from '../entity-state.model';

const INITIAL_STATE: State<CampoMatching> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function campoMatchingReducer(state = INITIAL_STATE, action: CampoMatchingActions): State<CampoMatching> {
  switch (action.type) {
    case actions.RESET:
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data.findIndex(b => b.idCampoMatching === state.selected.idCampoMatching
    ...
    case actions.DELETE_SUCCESS: {
      const data = state.data.filter(b => b.idCampoMatching !== state.selected.idCampoMatching
        || b.idConcTabla !== state.selected.idConcTabla);
      return {
        ...state,
        data,
