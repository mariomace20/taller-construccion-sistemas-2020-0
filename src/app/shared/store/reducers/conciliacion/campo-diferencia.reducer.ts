...
import { CampoDiferenciaActions, actions } from '../../actions/conciliacion/campo-diferencia.actions';
import { RESOURCE_ACTIONS } from '../../../utils';
import { State } from '../entity-state.model';

const INITIAL_STATE: State<CampoDiferencia> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function campoDiferenciaReducer(state = INITIAL_STATE, action: CampoDiferenciaActions): State<CampoDiferencia> {
  switch (action.type) {
    case actions.RESET:
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data.findIndex(b => b.idCampoDiferencia === state.selected.idCampoDiferencia
    ...
    case actions.DELETE_SUCCESS: {
      const data = state.data.filter(b => b.idCampoDiferencia !== state.selected.idCampoDiferencia
        || b.idConcTabla !== state.selected.idConcTabla);
      return {
        ...state,
        data,
