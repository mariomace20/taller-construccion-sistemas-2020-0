...
import { PaqueteActions, actions } from '../../actions/conciliacion/paquete.actions';
import { RESOURCE_ACTIONS } from '../../../utils';
import { State } from '../entity-state.model';

const INITIAL_STATE: State<Paquete> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function paqueteReducer(state = INITIAL_STATE, action: PaqueteActions): State<Paquete> {
  switch (action.type) {
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data.findIndex(b => b.idPaquete === state.selected.idPaquete);
      if (index >= 0) {
        const data = [
    ...
    case actions.DELETE_SUCCESS: {
      const data = state.data.filter(b => b.idPaquete !== state.selected.idPaquete);
      return {
        ...state,
        data,
        selected: null,
