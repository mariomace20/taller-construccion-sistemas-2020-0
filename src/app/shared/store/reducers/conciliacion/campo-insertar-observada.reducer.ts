...
import { CampoInsertarObservadaActions, actions } from '../../actions/conciliacion/campo-insertar-observada.actions';
import { RESOURCE_ACTIONS } from '../../../utils';
import { State } from '../entity-state.model';

const INITIAL_STATE: State<CampoInsertarObservada> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function campoInsertarObservadaReducer(state = INITIAL_STATE, action: CampoInsertarObservadaActions): State<CampoInsertarObservada> {
  switch (action.type) {
    case actions.RESET:
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data.findIndex(b => b.idCmpInsObservada === state.selected.idCmpInsObservada
    ...
    case actions.DELETE_SUCCESS: {
      const data = state.data.filter(b => b.idCmpInsObservada !== state.selected.idCmpInsObservada
        || b.idConcTabla !== state.selected.idConcTabla);
      return {
        ...state,
        data,
