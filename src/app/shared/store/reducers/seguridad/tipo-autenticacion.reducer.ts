...
import {actions, TipoAutenticacionActions} from "../../actions/seguridad/tipo-autenticacion.actions";
import {RESOURCE_ACTIONS} from "../../../utils";

const INITIAL_STATE: State<TipoAutenticacion> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function tipoAutenticacionReducer(state = INITIAL_STATE,
                                         action: TipoAutenticacionActions): State<TipoAutenticacion> {
  switch (action.type) {
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data.findIndex(
        tipoAutenticacion => tipoAutenticacion.idTipoAutenticacion === state.selected.idTipoAutenticacion);
      if (index >= 0) {
        const data = [
