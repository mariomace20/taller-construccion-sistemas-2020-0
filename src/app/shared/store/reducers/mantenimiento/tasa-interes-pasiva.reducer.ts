...
import {actions, TasaInteresPasivaActions} from '../../actions/mantenimiento/tasa-interes-pasiva.actions';

const INITIAL_STATE: State<TasaInteresPasiva> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function tasaInteresPasivaReducer(state = INITIAL_STATE, action: TasaInteresPasivaActions): State<TasaInteresPasiva> {
  switch (action.type) {
    case actions.RESET:
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data.findIndex(b => b.fechaProceso === state.selected.fechaProceso);
      if (index >= 0) {
        const data = [
    ...
    case actions.DELETE_SUCCESS: {
      const data = state.data.filter(b => b.fechaProceso !== state.selected.fechaProceso);
      return {
        ...state,
        data,
        selected: null,
