...
import { IndLimitePisoActions, actions } from '../../actions/mantenimiento/ind-limite-piso.actions';
import { RESOURCE_ACTIONS } from '../../../utils';
import { State } from '../entity-state.model';

const INITIAL_STATE: State<IndLimitePiso> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function indLimitePisoReducer(state = INITIAL_STATE, action: IndLimitePisoActions): State<IndLimitePiso> {
  switch (action.type) {
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data.findIndex(b => b.idLimitePiso === state.selected.idLimitePiso);
      if (index >= 0) {
        const data = [
    ...
    case actions.DELETE_SUCCESS: {
      const data = state.data.filter(b => b.idLimitePiso !== state.selected.idLimitePiso);
      return {
        ...state,
        data,
        selected: null,
