...
import { AtmRedAsociadaActions, actions } from '../../actions/mantenimiento/atm-red-asociada.actions';
import { RESOURCE_ACTIONS } from '../../../utils';
import { State } from '../entity-state.model';

const INITIAL_STATE: State<AtmRedAsociada> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function atmRedAsociadaReducer(state = INITIAL_STATE, action: AtmRedAsociadaActions): State<AtmRedAsociada> {
  switch (action.type) {
    case actions.RESET:
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data.findIndex(b => b.idInstitucion === state.selected.idInstitucion
        && b.idATM === state.selected.idATM);
      if (index >= 0) {
        const data = [
    ...
    case actions.DELETE_SUCCESS: {
      const data = state.data.filter(b => b.idInstitucion !== state.selected.idInstitucion
        || b.idATM !== state.selected.idATM);
      return {
        ...state,
        data,
