...
import { MetodoIdThbActions, actions } from '../../actions/mantenimiento/metodo-id-thb.actions';
import { RESOURCE_ACTIONS } from '../../../utils';
import { State } from '../entity-state.model';

const INITIAL_STATE: State<MetodoIdThb> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function metodoIdThbReducer(state = INITIAL_STATE, action: MetodoIdThbActions): State<MetodoIdThb> {
  switch (action.type) {
    case actions.RESET:
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data.findIndex(b => b.idMetodoIdThb === state.selected.idMetodoIdThb
    ...
    case actions.DELETE_SUCCESS: {
      const data = state.data.filter(b => b.idMetodoIdThb !== state.selected.idMetodoIdThb
        || b.idMembresia !== state.selected.idMembresia);
      return {
        ...state,
        data,
