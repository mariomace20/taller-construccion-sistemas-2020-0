...
import { CanalActions, actions } from '../../actions/mantenimiento/canal.actions';
import { RESOURCE_ACTIONS } from '../../../utils';
import { State } from '../entity-state.model';

const INITIAL_STATE: State<Canal> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function canalReducer(state = INITIAL_STATE, action: CanalActions): State<Canal> {
  switch (action.type) {
    case actions.RESET:
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data.findIndex(b => b.idCanal === state.selected.idCanal);
      if (index >= 0) {
        const data = [
    ...
    case actions.DELETE_SUCCESS: {
      const data = state.data.filter(b => b.idCanal !== state.selected.idCanal);
      return {
        ...state,
        data,
        selected: null,
