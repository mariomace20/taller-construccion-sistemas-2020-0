...
import { RegionActions, actions } from '../../actions/mantenimiento/region.actions';
import { RESOURCE_ACTIONS } from '../../../utils';
import { State } from '../entity-state.model';

const INITIAL_STATE: State<Region> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function regionReducer(state = INITIAL_STATE, action: RegionActions): State<Region> {
  switch (action.type) {
    case actions.RESET:
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data.findIndex(b => b.idRegion === state.selected.idRegion);
      if (index >= 0) {
        const data = [
    ...
    case actions.DELETE_SUCCESS: {
      const data = state.data.filter(b => b.idRegion !== state.selected.idRegion);
      return {
        ...state,
        data,
        selected: null,
