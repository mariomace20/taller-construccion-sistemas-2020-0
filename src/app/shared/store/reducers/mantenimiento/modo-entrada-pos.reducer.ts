...
import { ModoEntradaPosActions, actions } from '../../actions/mantenimiento/modo-entrada-pos.actions';
import { RESOURCE_ACTIONS } from '../../../utils';
import { State } from '../entity-state.model';

const INITIAL_STATE: State<ModoEntradaPos> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function modoEntradaPosReducer(state = INITIAL_STATE, action: ModoEntradaPosActions): State<ModoEntradaPos> {
  switch (action.type) {
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data.findIndex(b => b.idModoEntradaPOS === state.selected.idModoEntradaPOS
    ...
    case actions.DELETE_SUCCESS: {
      const data = state.data.filter(b => b.idModoEntradaPOS !== state.selected.idModoEntradaPOS
        || b.idMembresia !== state.selected.idMembresia);
      return {
        ...state,
        data,
