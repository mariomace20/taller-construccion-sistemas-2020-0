...
import { ProductoActions, actions } from '../../actions/mantenimiento/producto.actions';
import { RESOURCE_ACTIONS } from '../../../utils';
import { State } from '../entity-state.model';

const INITIAL_STATE: State<Producto> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function productoReducer(state = INITIAL_STATE, action: ProductoActions): State<Producto> {
  switch (action.type) {
    case actions.RESET:
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data.findIndex(b => b.idProducto === state.selected.idProducto);
      if (index >= 0) {
        const data = [
    ...
    case actions.DELETE_SUCCESS: {
      const data = state.data.filter(b => b.idProducto !== state.selected.idProducto);
      return {
        ...state,
        data,
        selected: null,
