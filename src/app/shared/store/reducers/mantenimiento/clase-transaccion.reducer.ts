...
import { ClaseTransaccionActions, actions } from '../../actions/mantenimiento/clase-transaccion.action';
import { RESOURCE_ACTIONS } from '../../../utils';
import { State } from '../entity-state.model';

const INITIAL_STATE: State<ClaseTransaccion> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function claseTransaccionReducer(state = INITIAL_STATE, action: ClaseTransaccionActions): State<ClaseTransaccion> {
  switch (action.type) {
    case actions.RESET:
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data
        .findIndex(b => b.idClaseTransaccion === state.selected.idClaseTransaccion);
      if (index >= 0) {
        const data = [
    ...
    case actions.DELETE_SUCCESS: {
      const data = state.data
        .filter(b => b.idClaseTransaccion !== state.selected.idClaseTransaccion);
      return {
        ...state,
        data,
        selected: null,
