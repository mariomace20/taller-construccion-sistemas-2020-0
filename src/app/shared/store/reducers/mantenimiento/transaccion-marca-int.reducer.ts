...
import { TransaccionMarcaIntActions, actions } from '../../actions/mantenimiento/transaccion-marca-int.actions';
import { RESOURCE_ACTIONS } from '../../../utils';
import { State } from '../entity-state.model';

const INITIAL_STATE: State<TransaccionMarcaInt> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function transaccionMarcaIntReducer(state = INITIAL_STATE, action: TransaccionMarcaIntActions): State<TransaccionMarcaInt> {
  switch (action.type) {
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data.findIndex(b => b.idMembresia === state.selected.idMembresia
        && b.idTransaccion === state.selected.idTransaccion);
      if (index >= 0) {
        const data = [
