...
import { CodigoRazonCobroVisaActions, actions } from '../../actions/mantenimiento/codigo-razon-cobro-visa.actions';
import { RESOURCE_ACTIONS } from '../../../utils';
import { State } from '../entity-state.model';

const INITIAL_STATE: State<CodigoRazonCobroVisa> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function codigoRazonCobroVisaReducer(state = INITIAL_STATE, action: CodigoRazonCobroVisaActions): State<CodigoRazonCobroVisa> {
  switch (action.type) {
    case actions.RESET:
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data.findIndex(b => b.idCodigoRazon === state.selected.idCodigoRazon);
      if (index >= 0) {
        const data = [
    ...
    case actions.DELETE_SUCCESS: {
      const data = state.data.filter(b => b.idCodigoRazon !== state.selected.idCodigoRazon);
      return {
        ...state,
        data,
        selected: null,
