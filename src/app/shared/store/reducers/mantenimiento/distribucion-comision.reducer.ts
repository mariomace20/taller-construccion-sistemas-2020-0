...
import { DistribucionComisionActions, actions } from '../../actions/mantenimiento/distribucion-comision.actions';
import { RESOURCE_ACTIONS } from '../../../utils';
import { State } from '../entity-state.model';

const INITIAL_STATE: State<DistribucionComision> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function distribucionComisionReducer(state = INITIAL_STATE, action: DistribucionComisionActions): State<DistribucionComision> {
  switch (action.type) {
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data
        .findIndex(b => b.idDistribucionComision === state.selected.idDistribucionComision);
      if (index >= 0) {
        const data = [
    ...
    case actions.DELETE_SUCCESS: {
      const data = state.data
        .filter(b => b.idDistribucionComision !== state.selected.idDistribucionComision);
      return {
        ...state,
        data,
        selected: null,
