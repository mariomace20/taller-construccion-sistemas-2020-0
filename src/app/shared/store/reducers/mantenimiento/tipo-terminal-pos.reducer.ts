...
import { TipoTerminalPosActions, actions } from '../../actions/mantenimiento/tipo-terminal-pos.actions';
import { RESOURCE_ACTIONS } from '../../../utils';
import { State } from '../entity-state.model';

const INITIAL_STATE: State<TipoTerminalPos> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function tipoTerminalPosReducer(state = INITIAL_STATE, action: TipoTerminalPosActions): State<TipoTerminalPos> {
  switch (action.type) {
    case actions.RESET:
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data.findIndex(b => b.idTerminalPOS === state.selected.idTerminalPOS
    ...
    case actions.DELETE_SUCCESS: {
      const data = state.data.filter(b => b.idTerminalPOS !== state.selected.idTerminalPOS
        || b.idMembresia !== state.selected.idMembresia);
      return {
        ...state,
        data,
