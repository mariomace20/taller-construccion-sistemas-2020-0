...
import { CodigoProcesoSwitchActions, actions } from '../../actions/mantenimiento/codigo-proceso-switch.actions';
import { RESOURCE_ACTIONS } from '../../../utils';
import { State } from '../entity-state.model';

const INITIAL_STATE: State<CodigoProcesoSwitch> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function codigoProcesoSwitchReducer(state = INITIAL_STATE, action: CodigoProcesoSwitchActions): State<CodigoProcesoSwitch> {
  switch (action.type) {
    case actions.RESET:
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data.findIndex(b => b.idCodigoProcesoSwitch === state.selected.idCodigoProcesoSwitch);
      if (index >= 0) {
        const data = [
    ...
    case actions.DELETE_SUCCESS: {
      const data = state.data.filter(b => b.idCodigoProcesoSwitch !== state.selected.idCodigoProcesoSwitch);
      return {
        ...state,
        data,
        selected: null,
