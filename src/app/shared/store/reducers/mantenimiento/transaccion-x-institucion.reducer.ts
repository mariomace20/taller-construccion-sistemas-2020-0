...
import { TransaccionXinstitucionActions, actions } from '../../actions/mantenimiento/transaccion-x-institucion.action';
import { RESOURCE_ACTIONS } from '../../../utils';
import { State } from '../entity-state.model';

const INITIAL_STATE: State<TransaccionXinstitucion> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function txnXinstitucionReducer(state = INITIAL_STATE,
  action: TransaccionXinstitucionActions): State<TransaccionXinstitucion> {
  switch (action.type) {
    case actions.RESET:
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data
        .findIndex(b => b.idInstitucion === state.selected.idInstitucion
          ...
          && b.idCodigoTransaccion === state.selected.idCodigoTransaccion);
      if (index >= 0) {
        const data = [
