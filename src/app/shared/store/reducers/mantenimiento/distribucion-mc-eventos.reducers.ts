...
import { DistribucionMcEventosActions, actions } from '../../actions/mantenimiento/distribucion-mc-eventos.actions';
import { RESOURCE_ACTIONS } from '../../../utils';
import { State } from '../entity-state.model';

const INITIAL_STATE: State<DistribucionMcEventos> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function distribucionMcEventosReducer(state = INITIAL_STATE, action: DistribucionMcEventosActions): State<DistribucionMcEventos> {
  switch (action.type) {
    case actions.RESET:
