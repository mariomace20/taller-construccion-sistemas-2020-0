...
import { BinActions, actions } from '../../actions/mantenimiento/bin.actions';
import { RESOURCE_ACTIONS } from '../../../utils';
import { State } from '../entity-state.model';

const INITIAL_STATE: State<Bin> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function binReducer(state = INITIAL_STATE, action: BinActions): State<Bin> {
  switch (action.type) {
    case actions.RESET:
    ...
    case actions.GET_ALL_FAIL:
      ...
      case actions.GET_X_INSTITUCION:
        return {
          ...state,
          action: RESOURCE_ACTIONS.CONSULTA,
          loading: true,
      ...
      case actions.GET_X_INSTITUCION_SUCCESS:
        return {
          ...state,
          data: action.payload,
          loading: false,
      ...
      case actions.GET_X_INSTITUCION_FAIL:
        return {
          ...state,
          data: [],
          loading: false,
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data.findIndex(b => b.idBIN === state.selected.idBIN);
      if (index >= 0) {
        const data = [
    ...
    case actions.DELETE_SUCCESS: {
      const data = state.data.filter(b => b.idBIN !== state.selected.idBIN);
      return {
        ...state,
        data,
        selected: null,
