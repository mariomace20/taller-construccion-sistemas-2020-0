...
import { EventosTransaccionMastercardActions, actions } from '../../actions/mantenimiento/eventos-transaccion-mastercard.actions';
import { RESOURCE_ACTIONS } from '../../../utils';
import { State } from '../entity-state.model';

const INITIAL_STATE: State<EventosTransaccionMastercard> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function eventosTransaccionMastercardReducer(state = INITIAL_STATE, action: EventosTransaccionMastercardActions): State<EventosTransaccionMastercard> {
  switch (action.type) {
    case actions.RESET:
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data.findIndex(b => b.idEventoMc === state.selected.idEventoMc);
      if (index >= 0) {
        const data = [
    ...
    case actions.DELETE_SUCCESS: {
      const data = state.data.filter(b => b.idEventoMc !== state.selected.idEventoMc);
      return {
        ...state,
        data,
        selected: null,
