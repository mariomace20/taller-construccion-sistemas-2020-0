...
import { EspacioAcademicoActions, actions } from '../../actions/mantenimiento/espacio-academico.actions';
import { RESOURCE_ACTIONS } from '../../../utils';
import { State } from '../entity-state.model';

const INITIAL_STATE: State<EspacioAcademico> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function espacioAcademicoReducer(state = INITIAL_STATE, action: EspacioAcademicoActions): State<EspacioAcademico> {
  switch (action.type) {
    case actions.RESET:
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data
        .findIndex(b => b.idEspacioAcademico === state.selected.idEspacioAcademico);
      if (index >= 0) {
        const data = [
    ...
    case actions.DELETE_SUCCESS: {
      const data = state.data
        .filter(b => b.idEspacioAcademico !== state.selected.idEspacioAcademico);
      return {
        ...state,
        data,
        selected: null,
