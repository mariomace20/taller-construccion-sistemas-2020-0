...
import { SolicitanteActions, actions } from '../../actions/mantenimiento/solicitante.actions';
import { RESOURCE_ACTIONS } from '../../../utils';
import { State } from '../entity-state.model';

const INITIAL_STATE: State<Solicitante> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function solicitanteReducer(state = INITIAL_STATE, action: SolicitanteActions): State<Solicitante> {
  switch (action.type) {
    case actions.RESET:
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data
        .findIndex(b => b.idSolicitante === state.selected.idSolicitante);
      if (index >= 0) {
        const data = [
    ...
    case actions.DELETE_SUCCESS: {
      const data = state.data
        .filter(b => b.idSolicitante !== state.selected.idSolicitante);
      return {
        ...state,
        data,
        selected: null,
