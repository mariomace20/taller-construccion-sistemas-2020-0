...
import { CodigoRptaMembresiaActions, actions } from '../../actions/mantenimiento/codigo-rpta-membresia.actions';
import { RESOURCE_ACTIONS } from '../../../utils';
import { State } from '../entity-state.model';

const INITIAL_STATE: State<CodigoRptaMembresia> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function codigoRptaMembresiaReducer(state = INITIAL_STATE, action: CodigoRptaMembresiaActions): State<CodigoRptaMembresia> {
  switch (action.type) {
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data.findIndex(b => b.idRespuesta === state.selected.idRespuesta &&
    ...
    case actions.DELETE_SUCCESS: {
      const data = state.data.filter(b => b.idRespuesta !== state.selected.idRespuesta ||
        b.idMembresia !== state.selected.idMembresia);
      return {
        ...state,
        data,
