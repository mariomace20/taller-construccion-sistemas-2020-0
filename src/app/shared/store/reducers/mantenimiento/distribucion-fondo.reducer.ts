...
import { DistribucionFondoActions, actions } from '../../actions/mantenimiento/distribucion-fondo.actions';
import { RESOURCE_ACTIONS } from '../../../utils';
import { State } from '../entity-state.model';

const INITIAL_STATE: State<DistribucionFondo> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function distribucionFondoReducer(state = INITIAL_STATE, action: DistribucionFondoActions): State<DistribucionFondo> {
  switch (action.type) {
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data
        .findIndex(b => b.idDistribucionFondo === state.selected.idDistribucionFondo);
      if (index >= 0) {
        const data = [
    ...
    case actions.DELETE_SUCCESS: {
      const data = state.data
        .filter(b => b.idDistribucionFondo !== state.selected.idDistribucionFondo);
      return {
        ...state,
        data,
        selected: null,
