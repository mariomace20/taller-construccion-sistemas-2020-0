...
import { PaisActions, actions } from '../../actions/mantenimiento/pais.actions';
import { RESOURCE_ACTIONS } from '../../../utils';
import { State } from '../entity-state.model';

const INITIAL_STATE: State<Pais> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function paisReducer(state = INITIAL_STATE, action: PaisActions): State<Pais> {
  switch (action.type) {
    case actions.RESET:
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data.findIndex(b => b.idPais === state.selected.idPais);
      if (index >= 0) {
        const data = [
    ...
    case actions.DELETE_SUCCESS: {
      const data = state.data.filter(b => b.idPais !== state.selected.idPais);
      return {
        ...state,
        data,
        selected: null,
