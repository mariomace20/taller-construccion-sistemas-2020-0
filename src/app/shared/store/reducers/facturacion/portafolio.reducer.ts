...
import { PortafolioActions, actions } from '../../actions/facturacion/portafolio.actions';
import { RESOURCE_ACTIONS } from '../../../utils';
import { State } from '../entity-state.model';

const INITIAL_STATE: State<Portafolio> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function portafolioReducer(state = INITIAL_STATE, action: PortafolioActions): State<Portafolio> {
  switch (action.type) {
    case actions.RESET:
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data.findIndex(b => b.idPortafolio === state.selected.idPortafolio);
      if (index >= 0) {
        const data = [
    ...
    case actions.DELETE_SUCCESS: {
      const data = state.data.filter(b => b.idPortafolio !== state.selected.idPortafolio);
      return {
        ...state,
        data,
        selected: null,
