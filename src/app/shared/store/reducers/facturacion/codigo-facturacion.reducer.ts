...
import { CodigoFacturacionActions, actions } from '../../actions/facturacion/codigo-facturacion.actions';
import { RESOURCE_ACTIONS } from '../../../utils';
import { State } from '../entity-state.model';

const INITIAL_STATE: State<CodigoFacturacion> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function codigoFacturacionReducer(state = INITIAL_STATE, action: CodigoFacturacionActions): State<CodigoFacturacion> {
  switch (action.type) {
    case actions.RESET:
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data.findIndex(b => b.idCodigoFacturacion === state.selected.idCodigoFacturacion);
      if (index >= 0) {
        const data = [
    ...
    case actions.DELETE_SUCCESS: {
      const data = state.data.filter(b => b.idCodigoFacturacion !== state.selected.idCodigoFacturacion);
      return {
        ...state,
        data,
        selected: null,
