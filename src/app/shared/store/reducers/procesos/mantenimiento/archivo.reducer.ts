...
import { ArchivoActions, actions } from '../../../actions/procesos/mantenimiento/archivo.actions';
import { RESOURCE_ACTIONS } from '../../../../utils';
import { State } from '../../entity-state.model';

const INITIAL_STATE: State<Archivo> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function archivoReducer(state = INITIAL_STATE, action: ArchivoActions): State<Archivo> {
  switch (action.type) {
    ...
    case actions.UPDATE_SUCCESS: {
      const index = state.data.findIndex(b => b.idArchivo === state.selected.idArchivo);
      if (index >= 0) {
        const data = [
    ...
    case actions.DELETE_SUCCESS: {
      const data = state.data.filter(b => b.idArchivo !== state.selected.idArchivo);
      return {
        ...state,
        data,
        selected: null,
