...
import { RangoBinVisaActions, actions } from '../../actions/consultas/rango-bin-visa.actions';
import { RESOURCE_ACTIONS } from '../../../utils';
import { ConsultaState } from '../entity-state.model';

const INITIAL_STATE: ConsultaState<RangoBinVisa> = {
  data: [],
  selected: null,
  action: null,
  loading: false,
...
export function rangoBinVisaReducer(state = INITIAL_STATE, action: RangoBinVisaActions): ConsultaState<RangoBinVisa> {
  switch (action.type) {
    case actions.RESET:
      return {
      ...INITIAL_STATE
